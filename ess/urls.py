...
urlpatterns = [
    ...
    url('^(?P<brand>\w+)/(?P<chassis>\w+)/$', CarModelListView.as_view(),
        name='car-model-list'),
    url('^(?P<brand>\w+)/(?P<chassis>\w+)/(?P<car_model>[\w\.]+)/$', ECUListView.as_view(),
        name='ecu-list'),
    url('^(?P<brand>\w+)/(?P<chassis>\w+)/(?P<car_model>[\w\.]+)/(?P<ecu>\w+)/$',
        ModificationListView.as_view(), name='modification-list'),
